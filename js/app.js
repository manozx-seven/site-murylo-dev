// === 0. FIX DE SCROLL (Sempre carregar no topo) ===
        ...
        const servicesData = [
            {
                ...
                shortDesc: 'Centralize todos os seus links em uma página profissional e atrativa.',
            ...
            },
            {
                ...
                fullDesc: 'Formulários personalizados que organizam todas as respostas automaticamente. Perfeito para receber pedidos, inscrições, pesquisas e cadastros sem perder nenhuma informação.',
                hasModels: false,
            ...
            },
            {
                ...
                shortDesc: 'Organização via Google Sheets com integrações poderosas.',
                fullDesc: 'Criação de dashboards, fórmulas complexas e automações (via Apps Script ou Make/Zapier) para economizar horas do seu dia.',
                hasModels: false,
            ...
            },
            {
                ...
                shortDesc: 'Transformo seu design (Figma/Adobe XD) em código perfeito.',
                fullDesc: 'Serviço para agências e designers: Você entrega a interface, eu programo com HTML/CSS/JS limpo, rápido e responsivo.',
                hasModels: false,
            ...
            },
            {
                ...
                shortDesc: 'Configuração de e-mails @suaempresa.com.br.',
                fullDesc: 'Configuração de domínio, apontamentos DNS e criação de caixas de e-mail profissionais (Google Workspace, Titan ou cPanel).',
                hasModels: false,
                iconBg: 'from-slate-100 to-gray-200',
        ...
        function updateBtnWa() {
            ...
            if(ready) {
                ...
                if(s.hasModels && currentState.model && currentState.plan) {
                    ...
                    const planPriceText = planEl ? planEl.querySelector('.text-brand-600.font-bold')?.textContent.replace('R$','').trim() : '';
                    const planPrice = parseFloat(planPriceText);
                    
                    if(!isNaN(modelPrice) && !isNaN(planPrice)) {
                        const total = modelPrice + planPrice;
                        btnWaText.textContent = `Solicitar Orçamento — Total: R$ ${total}`;
                    } else {
        ...
        const portfolioProjects = [
            ...
            {
                title: 'Sâmela Matozinho',
                badge: 'Site Institucional',
                desc: 'Site profissional desenvolvido para consultora acadêmica especializada em orientação de TCC, artigos e formatação ABNT. O projeto inclui página de serviços, depoimentos de alunos, formulário de contato integrado ao WhatsApp e otimização para aparecer nas buscas do Google em Manaus.',
        ...
        function openPortfolioModal(index) {
            ...
            if (project.type === 'biolink-models') {
                ...
                if (project.images.length === 1) {
                    ...
                    imgContainer.innerHTML = `
                        <div class="img-zoom-container w-full rounded-2xl border border-gray-100 shadow-sm bg-gray-50 relative" onmousemove="handleImgZoom(event)" onmouseleave="resetImgZoom(this)">
                    ...
                    imgContainer.innerHTML = `
                        <div class="img-zoom-container w-full rounded-2xl border border-gray-100 shadow-sm bg-gray-50 relative" onmousemove="handleImgZoom(event)" onmouseleave="resetImgZoom(this)">
                            ...
                            <button onclick="pfSlide(-1)" class="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/90 hover:bg-white rounded-full shadow-lg flex items-center justify-center text-gray-700 hover:text-brand-600 transition-colors z-10">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                            </button>
                            <button onclick="pfSlide(1)" class="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/90 hover:bg-white rounded-full shadow-lg flex items-center justify-center text-gray-700 hover:text-brand-600 transition-colors z-10">
        ...
        function handleImgZoom(e) {
            ...
            // Mover o transform-origin para onde o mouse está
            img.style.transformOrigin = `${percentX}% ${percentY}%`;

            // Posicionar a lente centralizada no cursor
